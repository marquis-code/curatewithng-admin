import { ref, defineComponent, computed, unref, withCtx, openBlock, createBlock, createVNode, toDisplayString, createCommentVNode, Fragment, renderList, useSSRContext } from "vue";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><div class="flex gap-3"><select class="input-field !py-2 !w-48"><option value=""${ssrIncludeBooleanAttr(Array.isArray(statusFilter.value) ? ssrLooseContain(statusFilter.value, "") : ssrLooseEqual(statusFilter.value, "")) ? " selected" : ""}>All Statuses</option><option value="APPROVED"${ssrIncludeBooleanAttr(Array.isArray(statusFilter.value) ? ssrLooseContain(statusFilter.value, "APPROVED") : ssrLooseEqual(statusFilter.value, "APPROVED")) ? " selected" : ""}>Approved</option><option value="PENDING"${ssrIncludeBooleanAttr(Array.isArray(statusFilter.value) ? ssrLooseContain(statusFilter.value, "PENDING") : ssrLooseEqual(statusFilter.value, "PENDING")) ? " selected" : ""}>Pending</option></select></div></div><div class="overflow-x-auto"><table class="w-full text-left border-collapse"><thead><tr class="bg-white border-b border-slate-200 text-sm font-semibold text-slate-600"><th class="p-4">Gift Info</th><th class="p-4">Vendor</th><th class="p-4">Price</th><th class="p-4">Status</th><th class="p-4 text-right">Actions</th></tr></thead><tbody>`);
      if (unref(loading)) {
        _push(`<!--[-->`);
        ssrRenderList(5, (i) => {
        ...
        ssrRenderList(filteredGifts.value, (gift) => {
          ...
          _push(`</div><div><div class="font-medium text-slate-900 line-clamp-1">${ssrInterpolate(gift.name)}</div><div class="text-xs text-slate-500 capitalize">${ssrInterpolate(gift.category || "Uncategorized")} • ${ssrInterpolate(gift.budgetTier)}</div></div></div></td><td class="p-4 text-sm text-slate-600 font-medium">${ssrInterpolate(gift.vendorId?.businessName || "Unknown Vendor")}</td><td class="p-4 font-bold text-slate-900">₦${ssrInterpolate(gift.price?.toLocaleString() || 0)}</td><td class="p-4"><span class="${ssrRenderClass(gift.isApproved ? "badge-active" : "badge-pending")}">${ssrInterpolate(gift.isApproved ? "APPROVED" : "PENDING")}</span>`);
          if (gift.isFeatured) {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            if (selectedGift.value) {
              ...
              if (selectedGift.value.images?.length > 1) {
              ...
              if (selectedGift.value.recipientTypes?.length) {
            ...
            return [
              selectedGift.value ? (openBlock(), createBlock("div", {
                ...
                createVNode("div", { class: "w-full h-56 bg-slate-100 rounded-2xl overflow-hidden border border-slate-200 flex items-center justify-center relative group" }, [
                  ...
                  selectedGift.value.images?.length > 1 ? (openBlock(), createBlock("div", {
                    key: 2,
                ...
                createVNode("div", { class: "grid grid-cols-2 gap-4" }, [
                  createVNode("div", { class: "bg-slate-50 p-4 rounded-xl border border-slate-100" }, [
                    ...
                    createVNode("div", { class: "font-bold text-slate-900 text-lg" }, "₦" + toDisplayString(selectedGift.value.price?.toLocaleString() || 0), 1)
                  ]),
                  createVNode("div", { class: "bg-slate-50 p-4 rounded-xl border border-slate-100" }, [
                ...
                createVNode("div", { class: "space-y-4" }, [
                  ...
                  selectedGift.value.occasions?.length ? (openBlock(), createBlock("div", { key: 0 }, [
                  ...
                  selectedGift.value.recipientTypes?.length ? (openBlock(), createBlock("div", { key: 1 }, [
                ...
                createVNode("div", { class: "bg-slate-50 p-4 rounded-xl border border-slate-100" }, [
                  ...
                  createVNode("div", { class: "flex items-center gap-3" }, [
                    createVNode("div", { class: "w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center font-bold text-slate-600" }, toDisplayString(selectedGift.value.vendorId?.businessName?.charAt(0) || "V"), 1),
                    createVNode("div", null, [
                      createVNode("div", { class: "font-bold text-slate-900" }, toDisplayString(selectedGift.value.vendorId?.businessName || "Unknown Vendor"), 1),
                      createVNode("div", { class: "text-xs text-slate-500" }, "@" + toDisplayString(selectedGift.value.vendorId?.slug || "slug-unavailable"), 1)
                    ])
                  ])
                ]),
...
//# sourceMappingURL=index-DUTHM_rk.js.map
