import { defineComponent, ref, computed, resolveComponent, unref, withCtx, openBlock, createBlock, createVNode, toDisplayString, Fragment, renderList, createCommentVNode, useSSRContext } from "vue";
    ...
    const filteredVendors = computed(() => {
      ...
      if (searchQuery.value) {
        ...
        result = result.filter(
          (v) => v.businessName?.toLowerCase().includes(q) || v.userId?.email?.toLowerCase().includes(q) || v.slug?.toLowerCase().includes(q)
        );
      }
      return result;
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        ssrRenderList(filteredVendors.value, (vendor) => {
          _push(`<tr class="border-b border-slate-100 bg-white hover:bg-slate-50 transition-colors"><td class="p-4"><div class="flex items-center gap-3"><div class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-600 uppercase">${ssrInterpolate(vendor.businessName?.substring(0, 2) || vendor.slug?.substring(0, 2) || "V")}</div><div><div class="font-medium text-slate-900">${ssrInterpolate(vendor.businessName || "Unnamed Vendor")}</div><div class="text-xs text-slate-500">@${ssrInterpolate(vendor.slug || "slug-unavailable")}</div></div></div></td><td class="p-4 text-sm text-slate-600">${ssrInterpolate(vendor.userId?.email || "No email attached")}</td><td class="p-4"><span class="${ssrRenderClass(vendor.isApproved ? "badge-active" : "badge-pending")}">${ssrInterpolate(vendor.isApproved ? "APPROVED" : "PENDING")}</span></td><td class="p-4 text-sm text-slate-600">${ssrInterpolate(new Date(vendor.createdAt).toLocaleDateString())}</td><td class="p-4 text-right"><button class="text-primary-600 hover:text-primary-800 font-medium text-sm border border-primary-200 px-3 py-1 rounded-lg hover:bg-primary-50 transition-colors"> Review </button></td></tr>`);
        });
        _push(`<!--]-->`);
      }
            ...
            return [
              selectedVendor.value ? (openBlock(), createBlock("div", {
                ...
                createVNode("div", { class: "flex items-center gap-4 border-b border-slate-100 pb-6" }, [
                  createVNode("div", { class: "w-16 h-16 rounded-2xl bg-primary-50 text-primary-800 flex items-center justify-center text-2xl font-bold uppercase" }, toDisplayString(selectedVendor.value.businessName?.charAt(0) || selectedVendor.value.slug?.charAt(0) || "V"), 1),
                  createVNode("div", null, [
                    ...
                    createVNode("p", { class: "text-slate-500 text-xs mt-1" }, toDisplayString(selectedVendor.value.userId?.email), 1)
                  ])
                ]),
                ...
                createVNode("div", { class: "grid grid-cols-2 gap-4" }, [
                  createVNode("div", { class: "bg-emerald-50 p-4 rounded-xl border border-emerald-100 text-center" }, [
                    ...
                    createVNode("div", { class: "font-bold text-emerald-900 text-xl" }, "₦" + toDisplayString(selectedVendor.value.totalEarnings?.toLocaleString() || 0), 1)
                  ]),
                  createVNode("div", { class: "bg-amber-50 p-4 rounded-xl border border-amber-100 text-center" }, [
                    ...
                    createVNode("div", { class: "font-bold text-amber-900 text-xl" }, "₦" + toDisplayString(selectedVendor.value.pendingPayout?.toLocaleString() || 0), 1)
                  ])
                ]),
                createVNode("div", { class: "space-y-4 pt-4 border-t border-slate-100" }, [
                  ...
                  createVNode("div", { class: "text-sm text-slate-600 bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-3" }, [
                    ...
                    selectedVendor.value.categories?.length > 0 ? (openBlock(), createBlock("div", {
                      key: 0,
                      class: "pt-2"
                    }, [
...
//# sourceMappingURL=index-DOLN7U3m.js.map
