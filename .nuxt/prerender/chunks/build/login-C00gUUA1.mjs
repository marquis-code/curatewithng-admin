...
import { defineComponent, ref, computed, mergeProps, unref, withCtx, createVNode, useId, useSSRContext } from 'file:///Users/marquis/curatewithng/admin/node_modules/vue/index.mjs';
import { ssrRenderAttrs, ssrRenderComponent, ssrInterpolate, ssrIncludeBooleanAttr, ssrRenderList, ssrRenderAttr, ssrRenderSlot, ssrRenderClass } from 'file:///Users/marquis/curatewithng/admin/node_modules/vue/server-renderer/index.mjs';
import { Check, Shield, Mail, Lock, Loader2, ArrowRight, KeyRound, Eye, EyeOff } from 'file:///Users/marquis/curatewithng/admin/node_modules/lucide-vue-next/dist/cjs/lucide-vue-next.js';
import { G as GATEWAY_ENDPOINT_WITH_AUTH, a as GATEWAY_ENDPOINT } from './axios.config-DHAaROs1.mjs';
...
const _imports_0 = "" + buildAssetsURL("auth-bg.BAHkeY5e.png");
const auth_api = {
  login: (payload) => {
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen grid lg:grid-cols-2 bg-white" }, _attrs))} data-v-9a5a51dd><div class="relative flex flex-col justify-center px-6 sm:px-16 lg:px-24 py-12" data-v-9a5a51dd>`);
      if (notification.value) {
        _push(`<div class="absolute top-8 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-emerald-50 px-6 py-3 rounded-full border border-emerald-200" data-v-9a5a51dd><div class="w-6 h-6 rounded-full bg-emerald-200 flex items-center justify-center text-emerald-700" data-v-9a5a51dd>`);
        _push(ssrRenderComponent(unref(Check), { class: "w-3.5 h-3.5" }, null, _parent));
        _push(`</div><p class="text-sm font-semibold text-emerald-800" data-v-9a5a51dd>${ssrInterpolate(notification.value)}</p></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="max-w-md w-full mx-auto" data-v-9a5a51dd><div class="flex items-center justify-center gap-3 mb-12" data-v-9a5a51dd><div class="w-10 h-10 bg-slate-900 text-white flex items-center justify-center rounded-lg" data-v-9a5a51dd>`);
      _push(ssrRenderComponent(unref(Shield), { class: "w-5 h-5" }, null, _parent));
      _push(`</div><div class="font-heading font-extrabold text-xl tracking-tight text-slate-900" data-v-9a5a51dd> Curate<span class="text-accent-500" data-v-9a5a51dd>With</span>NG <span class="text-slate-400 font-medium tracking-normal ml-1 border-l border-slate-200 pl-2" data-v-9a5a51dd>Admin</span></div></div>`);
      if (step.value === "credentials") {
        _push(`<div data-v-9a5a51dd><h1 class="text-3xl font-heading font-extrabold text-slate-900 mb-2" data-v-9a5a51dd>Secure Login</h1><p class="text-slate-500 text-base mb-10" data-v-9a5a51dd>Enter your credentials to access the command center.</p><form class="space-y-6" data-v-9a5a51dd>`);
        ...
        _push(`<button type="submit"${ssrIncludeBooleanAttr(loading.value) ? " disabled" : ""} class="w-full flex justify-center items-center gap-2 py-3.5 mt-2 bg-slate-900 text-white font-semibold rounded-xl hover:bg-slate-800 transition-colors" data-v-9a5a51dd>`);
        if (loading.value) {
          _push(ssrRenderComponent(unref(Loader2), { class: "w-5 h-5 animate-spin" }, null, _parent));
        } else {
          _push(`<span data-v-9a5a51dd>Continue securely</span>`);
        }
        if (!loading.value) {
        ...
        _push(`<div data-v-9a5a51dd><div class="w-12 h-12 bg-slate-100 text-slate-700 rounded-xl flex items-center justify-center mb-6" data-v-9a5a51dd>`);
        _push(ssrRenderComponent(unref(KeyRound), { class: "w-6 h-6" }, null, _parent));
        _push(`</div><h1 class="text-3xl font-heading font-extrabold text-slate-900 mb-2" data-v-9a5a51dd>Two-Factor Auth</h1><p class="text-slate-500 text-base mb-10" data-v-9a5a51dd>We&#39;ve sent a 6-digit code to <strong data-v-9a5a51dd>${ssrInterpolate(email.value)}</strong></p><form class="space-y-6" data-v-9a5a51dd><div data-v-9a5a51dd><label class="text-sm font-semibold text-slate-700 mb-3 block" data-v-9a5a51dd>Verification Code</label><div class="flex gap-3 justify-between" data-v-9a5a51dd><!--[-->`);
        ssrRenderList(otpDigits.value, (digit, index) => {
          _push(`<input${ssrRenderAttr("value", otpDigits.value[index])} type="text" inputmode="numeric" maxlength="1" class="w-12 h-14 rounded-xl border border-slate-200 bg-white text-slate-900 text-center text-2xl font-semibold focus:outline-none focus:border-slate-900 focus:ring-1 focus:ring-slate-900 transition-all" data-v-9a5a51dd>`);
        });
        _push(`<!--]--></div></div><button type="submit"${ssrIncludeBooleanAttr(loading.value) ? " disabled" : ""} class="w-full flex justify-center items-center gap-2 py-3.5 bg-slate-900 text-white font-semibold rounded-xl hover:bg-slate-800 transition-colors" data-v-9a5a51dd>`);
        if (loading.value) {
          _push(ssrRenderComponent(unref(Loader2), { class: "w-5 h-5 animate-spin" }, null, _parent));
        } else {
          _push(`<span data-v-9a5a51dd>Verify &amp; Login</span>`);
        }
        _push(`</button></form><div class="mt-8" data-v-9a5a51dd><button type="button" class="text-sm text-slate-500 hover:text-slate-900 font-semibold transition-colors flex items-center gap-1" data-v-9a5a51dd> \u2190 Back to login </button></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div><div class="hidden lg:flex items-center justify-center p-6 bg-slate-50" data-v-9a5a51dd><div class="relative w-full h-full overflow-hidden rounded-[2.5rem] bg-slate-900 shadow-[0_0_40px_-10px_rgba(0,0,0,0.05)]" data-v-9a5a51dd><img${ssrRenderAttr("src", _imports_0)} alt="Premium Admin Dashboard" class="absolute inset-0 w-full h-full object-cover opacity-80" data-v-9a5a51dd><div class="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/30 to-transparent" data-v-9a5a51dd></div><div class="absolute bottom-12 left-12 right-12 z-10" data-v-9a5a51dd><h2 class="font-heading font-extrabold text-4xl leading-tight mb-4 text-white" data-v-9a5a51dd> Command Central. </h2><p class="text-white/80 text-lg max-w-md" data-v-9a5a51dd> Curate and manage the ecosystem with absolute precision. </p></div></div></div></div>`);
    };
  }
});
...
const login = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-9a5a51dd"]]);

export { login as default };
//# sourceMappingURL=login-C00gUUA1.mjs.map
